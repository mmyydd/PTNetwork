		...
		int error_code,const char *errmsg)
{
	...
	result->error = gcmalloc_alloc(NULL, sizeof(DbError));
...
qboolean db_command_bind_param(struct db_conn *conn, DbQuery *pQuery, DbQueryResult *result, MYSQL_STMT *stmt)
{
	...
	bind_params = gcmalloc_alloc(NULL, sizeof(MYSQL_BIND) * pQuery->n_params);
...
void db_command_run_query(struct db_conn *conn, DbQuery *pQuery, DbQueryResult *result)
{
	...
	result->fields = gcmalloc_alloc(NULL, sizeof(DbFields));
	
	db_fields__init(result->fields);
	result->fields->n_fields = n_fields;
	result->fields->fields = gcmalloc_alloc(NULL, sizeof(DbField*) * n_fields);

	for( i = 0; i < n_fields; i++)
	{
		field = gcmalloc_alloc(NULL, sizeof(DbField));
	...
	//填充rows
	//
	bindResult = gcmalloc_alloc(NULL, sizeof(MYSQL_BIND) * n_fields);
	resultLength = gcmalloc_alloc(NULL, sizeof(unsigned long) * n_fields);
#define RESET_RESULT() for( j = 0; j < n_fields; j++) \
	...
	result->rows = gcmalloc_alloc(NULL, sizeof(DbRows));
	db_rows__init(result->rows);

	result->rows->n_rows = n_rows;
	result->rows->rows = gcmalloc_alloc(NULL, sizeof(DbRow*) * n_rows);
	...
	while(true)
	{
		...
		result->rows->rows[j] = gcmalloc_alloc(NULL, sizeof(DbRow));
		db_row__init(result->rows->rows[j]);

		result->rows->rows[j]->n_column = n_fields;
		result->rows->rows[j]->column = gcmalloc_alloc(NULL, sizeof(ProtobufCBinaryData) * n_fields);

		//横向填充column
		for( i = 0; i < n_fields; i++)
		{
			bindResult[i].buffer = gcmalloc_alloc(NULL, resultLength[i]);
			bindResult[i].buffer_length = resultLength[i];
			result->rows->rows[j]->column[i].len = bindResult[i].buffer_length;
			result->rows->rows[j]->column[i].data = (uint8_t*)bindResult[i].buffer;
...
{
	struct db_conn *conn;
	DbQueryResult *result = gcmalloc_alloc(NULL, sizeof(DbQueryResult));
