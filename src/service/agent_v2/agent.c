...
{
	struct pt_agent_user_info *userinfo;
	...
	userinfo = MEM_MALLOC(sizeof(struct pt_agent_user_info));
...
static void pt_agent_user_received(struct pt_sclient *user, struct pt_buffer *buff)
{
	...
	while(node)
	{
		...
		{
			//过滤非法数据包（用户未登陆）
			if(node->need_login == true && userinfo->is_login == false)
			{
				break;
			}
...
{
	struct pt_agent *agent = user->server->data;
	struct pt_agent_user_info *userinfo = (struct pt_agent_user_info*)user->data;
...
struct pt_agent *pt_agent_new()
{
	...
	pt_server_init(agent->server, uv_default_loop(), 10000, 120, pt_agent_user_connected, pt_agent_user_received, pt_agent_user_disconnected);
...
void pt_agent_add_server(struct pt_agent *agent, const char *name, uint32_t server_id, const char *host, uint16_t port, qboolean is_pipe, qboolean need_login)
{
