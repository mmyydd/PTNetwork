...
static qboolean pt_agent_user_connected(struct pt_sclient *user)
{
	...
	if(!pt_agent_is_connected(agent)){
...
void pt_agent_add_server(struct pt_agent *agent, const char *name, uint32_t server_id, const char *host, uint16_t port)
{
...
void pt_agent_node_try_connect(struct pt_agent_node *node)
{
	if(node->conn == PT_NO_CONNECT)
	{
		if( pt_client_connect(node->conn, node->host, node->port) == false )
		{
			private_WriteLog(ERROR_LEVEL_FATAL, __FUNCTION__, __FILE__,__LINE__,
...
qboolean pt_agent_is_active(struct pt_agent *agent)
{
...
void pt_agent_startup(struct pt_agent *agent)
{
